...
const Sidebar = ({ sidebarOpen, closeSidebar }) => {
	return (
		<div className='fixed z-50 flex w-screen mt-[60px] sm:hidden h-[calc(100vh_-_60px)]'>
			<motion.div className={'w-screen h-screen fixed bg-black'} animate={{ opacity: sidebarOpen ? 0.7 : 0 }} transition={{ duration: 0.5, type: 'tween' }}></motion.div>
			<div className={`w-screen h-screen fixed ${sidebarOpen ? 'block' : 'hidden'}`} onClick={closeSidebar}></div>
